...
- **异步支持**：Splash 中的 `await` 语法，用于网络请求等异步操作，当前需要通过 Rust 端桥接。
...
- **增量更新协议**：目前每次 `script_eval` 替换整个 widget tree。计划支持针对特定 widget 的局部更新（`script_patch`），减少重建开销。
...
Makepad 2.0 的核心创新不是某个具体技术，而是一个架构模式：**将 UI 定义从编译期搬到运行时，用独立脚本语言描述，通过流式求值即时渲染。**
