...
`script_mod!` 是整个应用中最重要的部分。花括号内的代码不是 Rust——它是 Splash 脚本。Rust 编译器会把它当作字符串字面量保留，在运行时由 Splash VM 解析和执行。
...
`on_render` 是一个渲染回调——在每次调用 `render()` 时重新执行。`counter_label` 不是固定的 Widget，而是每次渲染时动态创建的。当 `state.counter` 改变后，调用 `ui.main_view.render()` 会触发 `on_render` 重新执行，Label 的 `text` 属性被设置为新值。这种"状态变化 → 重新渲染 → UI 更新"的模式是 Makepad 2.0 中 UI 更新的标准方式。
...
这是怎么做到的？因为 `script_mod!` 内的代码**不是在 Rust 编译时处理的**。Rust 编译器只是把它当作一个字符串字面量保留。在运行时，Splash VM 解析这个字符串，构建 Widget 树。当源文件变化时，VM 重新解析修改后的字符串，重新构建 Widget 树——整个过程不涉及 Rust 编译器。
