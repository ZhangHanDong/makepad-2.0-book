...
| 框架 | 热重载粒度 | 延迟 | 保持状态？ |
|------|-----------|------|-----------|
| React HMR | 模块级 | ~200ms | 部分保持 |
| Flutter 热重载 | Widget 级 | ~300ms | 保持 |
| SwiftUI Preview | 整页重建 | ~1-2s | 不保持 |
| QML 热重载 | QML 文件级 | ~100ms | 部分保持 |
| egui | 需重编译 | ~5-30s | 不保持 |
| **Makepad Splash** | **表达式级** | **<50ms** | **保持** |

Makepad 的热重载粒度是最细的——改一个属性值，50 毫秒内看到效果。这不只是"开发体验好"——当 AI 逐 token 输出 Splash 代码时，这种粒度意味着用户可以看到 UI 逐步成型的过程（详见第11章：流式求值）。
...
这个循环的每一步都需要时间。如果"渲染"需要编译+打包+加载（React/Flutter），循环的延迟是秒级。如果"渲染"是毫秒级的（Splash），AI 可以在一次交互中完成多轮迭代。

Makepad 2.0 的 Canvas 工具就是这个循环的实现：AI Agent 生成 Splash 代码 → 通过 WebSocket 推送到 Canvas → Canvas 立即渲染 → 截图回传给 Agent → Agent 分析并修改 → 重新推送。整个循环可以在 3 秒内完成（详见第29章：自愈循环与流式渲染）。
